...
def test_get_callbacks_is_tuner_false():
    with mock.patch("train_and_evaluate.slurm_job_id", "12345"):
        callbacks = get_callbacks(is_tuner=False)
        ...
        assert (
            ...
            == f"/data/mguevaral/jpedro/jobs/pheno_tr.12345/checkpoints/weights.h5"
        )
        ...
        assert (
            callbacks[2].log_dir == f"/data/mguevaral/jpedro/jobs/pheno_tr.12345/logs"
        )
...
def test_build_autoencoder_model_learning_rate():
    ...
    assert model.optimizer.learning_rate.initial_learning_rate == hp.get(
        "learning_rate"
    )
